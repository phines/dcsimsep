...
while t < t_max
    ...
    if opt.sim.use_control
        ...
        if exist(comm_status_file,'file')
            ...
            is_flow_readable = comm_status(F) | comm_status(T);
            measured_flow = nan(m,1);
            measured_flow(is_flow_readable) = flow(is_flow_readable);
        else
...
n_overloads = sum(ps.branch(:,C.br.Pf)>ps.branch(:,C.br.rateA));

if opt.verbose
    fprintf('-------------- t = %7.3f -----------------\n',t);
    fprintf(' Simulation complete\n');
    fprintf('  %d overloads remain\n',n_overloads);
